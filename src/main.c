...
unsigned int colors[COLOR_COUNT] = {
    0xb4befeff, 0x89b4faff, 0x74c7ecff, 0x89dcebff, 0x94e2d5ff, 0xa6e3a1ff, 0xf9e2afff,
...
Wheel wheel_new(
    Vector2 center, float radius, float max_speed, unsigned int slices, Color base_color
) {
    return (Wheel){
        center, radius, max_speed, slices, base_color, .rot_deg = 0, .velocity = 0,
    };
}
    ...
    for (int i = *wheel_count - 1; i >= 0; i--) {
        ...
        if (CheckCollisionPointCircle(GetMousePosition(), wheel->center, wheel->radius)) {
            ...
            if (IsKeyReleased(KEY_SPACE))
                wheel->velocity =
                    Lerp(wheel->max_speed + WHEEL_SPIN_FORCE, wheel->max_speed, WHEEL_ACCEL);
...
int main(void) {
    ...
    wheels_add(
        wheels, &wheel_count,
        wheel_new(
            (Vector2){ WINDOW_WIDTH / 2.0f, WINDOW_HEIGHT / 2.0f }, 200, 50, 100,
